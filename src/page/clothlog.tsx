...
import { QuestionCircleOutlined, RedoOutlined, ExclamationCircleOutlined, DeleteOutlined, ScissorOutlined, SaveOutlined } from '@ant-design/icons';
...
const ClothLog: React.FC<Props> = ({
  ...
  const saveReport = async () => {
    ...
    worksheet.columns =
      [
        { header: t('report.date'), key: 'timestamp', },
        { header: t('log.event'), key: 'event', },
        { header: t('log.length'), key: 'meters', },
      ];
    worksheet.duplicateRow(1, 4, true);
    addTitle(worksheet, t('menu.clothlog') + ' ' + lifetime?.type + ' (' + lifetime?.serialno + '）', (period ? period[0].format('L LTS') : dayjs().subtract(7, 'days').format('L LTS')) + ' - ' + (period ? period[1].format('L LTS') : dayjs().format('L LTS')))
    ...
    worksheet.addRows((data || []).map((record: any) => ({
      ...record,
      timestamp: (dayjs(record.timestamp['lower']).format('LL LTS')) + ' - ' + (record.timestamp['upper'] ? dayjs(record.timestamp['upper']).format('LL LTS') : `‪‪ `),
      event: modeCodeObj(record.event).text,
