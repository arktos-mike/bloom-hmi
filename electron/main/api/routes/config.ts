      ...
      if (opIP.wired) {
        if (opIP.wired.dhcp == false) {
          switch (process.platform) {
            case 'linux':
              sudo.exec("nmcli con del wired && nmcli con add con-name \"wired\" type $(nmcli device status | awk \'BEGIN {cnt=0} $2==\"ethernet\" {cnt++} END {print (cnt>=2? \"bridge\":\"ethernet\")}') ifname $(nmcli device status | awk \'$2==\"ethernet\"\' | sort | awk \'BEGIN {cnt=0} $2==\"ethernet\" {cnt++} END {print (cnt>=2? \"app-br0\":$1)}') ipv4.method manual ip4 " + opIP.wired.ip_address + "/" + maskToPrefixLength(opIP.wired.netmask) + " gw4 " + opIP.wired.gateway_ip + " && nmcli con down wired && nmcli con up wired", options, async (error, data, getter) => {
                if (!error) {
          ...
          switch (process.platform) {
            case 'linux':
              sudo.exec("nmcli con del wired && nmcli con add con-name \"wired\" type $(nmcli device status | awk \'BEGIN {cnt=0} $2==\"ethernet\" {cnt++} END {print (cnt>=2? \"bridge\":\"ethernet\")}') ifname $(nmcli device status | awk \'$2==\"ethernet\"\' | sort | awk \'BEGIN {cnt=0} $2==\"ethernet\" {cnt++} END {print (cnt>=2? \"app-br0\":$1)}') ipv4.method auto && nmcli con down wired && nmcli con up wired", options, async (error, data, getter) => {
                if (!error) {
                  await db.query('UPDATE hwconfig set data = jsonb_set(data, $2, $3) where name=$1', ['ipConf', '{opIP, wired, dhcp}', opIP.wired.dhcp]);
                  res.status(200).json({
                    message: "notifications.confupdate",
